...
$page_by_link = $pdo->prepare("SELECT `pages`.`name`,`pages`.`text`,`pages`.`template`,`pages`.`public_flag`,`pages`.`id`,`pages`.`parent` FROM `pages` WHERE `pages`.`link` = :page_link AND ( `pages`.`public_flag` = 1 OR :admin_flag );");
